...
Prerequisites:
install strip and install upx.
They are used to minimize the binary size.

1) Build the toggle tool.. used for toggling the HyprTile launcher window
-> build_sendtoggle.sh

2) Optional: Build the screenshot component (used for the website screenshotter (F10), only when this feature is needed)
The reason why this is separate is because the launcher feels more snappy when these kinds of things are separated.
-> build_screenshotd.sh

3) Finally build the launcher
-> build.sh

After building, download some icons, download jQuery, see the respective folders icons/ and js/
Edit the hyprland config as shown below.
Copy the app folder HyprTile into your home directory. cd into it and run ./Hyprtile
With a tap on Super+ALT_L Keys, you may toggle the launcher.
...
# Important Info:
1) a) Build it,
or b) just download this package, then check if sha256 checksum of the package is like it is displayed on https://hyprtile.org/
